...
class DeviceDataManager(IDataMessageListener):
	...
	def startManager(self):
		...
		try:
			...
			logging.error(f"Failed to start sysPerfMgr: {e}", exc_info=True)

		try:
			if self.sensorAdapterMgr:
				self.sensorAdapterMgr.startManager()
		except Exception as e:
			logging.error(f"Failed to start sensorAdapterMgr: {e}", exc_info=True)
	...
	def stopManager(self):
		...
		try:
			...
			logging.error(f"Failed to stop sysPerfMgr: {e}", exc_info=True)

		try:
			if self.sensorAdapterMgr:
				self.sensorAdapterMgr.stopManager()
		except Exception as e:
			logging.error(f"Failed to stop sensorAdapterMgr: {e}", exc_info=True)
	...
	def _handleSensorDataAnalysis(self, data: SensorData = None, resource = None,):
		"""
